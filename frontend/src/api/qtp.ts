...
export const qtp = {
  ...
  testsPage: (params: QueryParams = {}) => api.get<Page<Scenario>>(`/api/scenarios${qs(params)}`),
  tests: (query = "") => api.get<Page<Scenario>>(`/api/scenarios${qs({ page_size: 100, ...fromLegacyQuery(query) })}`).then(list<Scenario>()),
  test: (id: string) => api.get<TestDetail>(`/api/scenarios/${id}`),
  updateTestTags: (id: string, tags: string[]) => api.put<{ id: string; tags: string[] }>(`/api/scenarios/${id}/tags`, { tags }),
  testComments: (id: string) => api.get<{ items: CommentItem[] }>(`/api/scenarios/${id}/comments`).then(list<CommentItem>()),
  createTestComment: (id: string, body: string, tags: string[] = []) => api.post<CommentItem>(`/api/scenarios/${id}/comments`, { body, tags }),
  discover: () => api.post<any>("/api/scenarios/discover"),
  runTest: (id: string, environment = "default") => api.post<RunSummary>(`/api/scenarios/${id}/run`, { environment }),
  deleteTest: (id: string) => api.del<any>(`/api/scenarios/${id}`),
  ...
  restartRun: (id: string) => api.post<any>(`/api/runs/${id}/restart`),
  restartFailedRuns: () => api.post<any>("/api/runs/restart-failed"),
  deleteRun: (id: string) => api.del<any>(`/api/runs/${id}`),
  deleteAllRuns: () => api.del<any>("/api/runs"),
